...
import {TransactionEvent} from '../src/types';
...
describe('transaction', () => {
  ...
  test('transaction', () => {
    ...
    expect(R.pipe(R.dissoc('events'), R.dissoc('id'))(events2[0])).toEqual({
      path: ['a'],
      type: 'TRANSACTION',
    });
  ...
  test('uniq transaction events', () => {
    ...
    p.endTransaction();
    expect(R.map(R.pipe(R.dissoc('events'), R.dissoc('id')), events)).toEqual([
      {type: 'TRANSACTION', path: []},
    ]);
